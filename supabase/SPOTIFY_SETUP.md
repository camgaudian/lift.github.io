...
- `001_schema.sql` ΓÇö `user_now_playing`, `now_playing_reactions`
- `004_functions.sql` ΓÇö `set_now_playing`, `clear_now_playing`, `get_my_now_playing`, reaction RPCs, and `get_friend_summary` (includes friends' active songs)
...
3. Fill in:
   ...
   - **Redirect URI**: `https://example.com/callback` (required by the form; not used for search-only Client Credentials flow ΓÇö you can use any valid HTTPS URL)
4. Accept the terms and click **Save**.
5. Open your new app ΓåÆ **Settings** (gear icon).
6. Copy and save:
   - **Client ID**
...
2. Find your **Project ref** in Supabase ΓåÆ **Project Settings ΓåÆ General** (e.g. `abcdefghijklmnop`).
...
The app calls this via `supabase.functions.invoke('spotify-search', ΓÇª)` ΓÇö no frontend env changes needed.
...
2. Run the app:
   ...
   npm run dev
   ```
3. Log in ΓåÆ **Profile** ΓåÆ **What's powering your lift?**
...
| Function deploy fails | CLI not linked | Re-run `supabase link --project-ref ΓÇª` |
